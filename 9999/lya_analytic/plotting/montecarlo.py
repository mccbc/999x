...
from util import Line, read_bin, voigtx_fast
...
def monte_carlo(x, bins=64):
    ...
    norm = 1./np.sum(n)/(bins[1:]-bins[:-1])

    print(np.sum(n*norm))

    # Get variance of x within each bin's edges
#    variance = [np.var(x[(bins[i]<x) & (x<bins[i+1])]) for i in range(len(bins)-1)]

    phat = n/np.sum(n)
    print(phat)
    err = 1.96*np.sqrt(phat*(1.-phat)/np.sum(n))*np.sum(n)

    return bincenters, n*norm, err*norm

def hsp_analytic(x, line, temp=1e4, radius=1e11, L=1., tau0=1e7, xi=0.0):
    ...
    return Jprefac/(3.0*phix)/(kx*radius)**3.*(np.pi**2./2.0)/(1.0+np.cosh(np.pi*z))*norm
...
def h_bc(x, line):
    ...
    n = len(x)
    dsigma = 2.0*(beta*np.max(x)**3./a)/(n-1) # Ok to use analytic function for sigma here? Yes
    ...
    amp = linalg.solve(M,b,debug=True)
    check=np.dot(M,amp)
    print(np.allclose(np.dot(M,amp),b,rtol=1e-05,atol=0.e0))
...
if __name__=='__main__':
    ...
    ax.plot(xbins, hsp_analytic(xbins, lya), 'g-', linewidth=1, alpha=0.5, label=r'$H_0$')
